...
// ##################### Configuração Luz
// ##################### ID
//Serial.printf("idluz[0] %d\n",idLuz[0]);
//Serial.printf("EEPROM_idLuz0 %d\n",EEPROM.read(EEPROM_idLuz0));
  for(int p = EEPROM_idLuz0, x=0; p < (EEPROM_idLuz0+qtdLuz); p++, x++){
    if(EEPROM.read(p) >= 1){
      if(EEPROM.read(p) <= 50){
...
//      Serial.printf("[EEPROM LOAD] Atualizando idServer%s para %d\n", x, idServer[x]);
//      TelnetClient.printf("[EEPROM LOAD] Atualizando idServer%s para %d\n", x, idServer[x]);
//    }
//  }
...
// #############################################
// ##################### Configuração idClient

  for(int x = 0, y = EEPROM_idClient0; x < 20; x++, y++){
    SALVO_idClient[x] = EEPROM.read(y);
    if(SALVO_idClient[x] != 0 && SALVO_idClient[x] != 255){
      Serial.printf("[EEPROM LOAD] Atualizando SALVO_idClient[%d] para\t%d\n",x, SALVO_idClient[x]);  
    }
  }
